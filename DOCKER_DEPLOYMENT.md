...
```yaml
volumes:
  - /:/host:ro  # 挂载整个宿主机根目录到容器的/host目录
```

这样配置的优势：
- 代码在Docker和本地环境中行为一致
- 用户在Web界面中看到的是标准的Linux目录结构（从/开始）
- 无需为不同目录单独配置挂载点
...
Docker环境中，容器需要访问宿主机上的文件才能进行备份。有以下几种配置方式：
...
volumes:
  ...
  # 应用日志
  - /var/log:/backup-sources/var-log:ro
```
...
1. **两个容器必须有相同的挂载**: backup-web和rclone容器必须挂载相同的目录
2. **只读挂载**: 使用`:ro`标志确保容器只能读取文件，不能修改
3. **路径一致性**: 确保两个容器中的路径映射完全一致
4. **临时文件共享**: `./data/temp`目录在两个容器间共享，用于传递压缩文件
